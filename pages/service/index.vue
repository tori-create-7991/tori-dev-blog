<template>
  <div class="mx-auto max-w-screen-xl px-4 py-10">
    <h1 class="font-display text-2xl font-bold text-gray-900 mb-8">Service</h1>

    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <UCard v-for="item in services" :key="item.path">
        <NuxtLink :to="item.path" class="block">
          <span class="text-xs font-medium text-violet-800">{{ categoryLabel(item.category) }}</span>
          <h2 class="mt-1 font-semibold text-gray-900">{{ item.title }}</h2>
          <p class="mt-2 text-sm text-gray-600 line-clamp-3">{{ item.description }}</p>
        </NuxtLink>
      </UCard>
    </div>
  </div>
</template>

<script setup>
import { serviceCategoryLabel } from '~/siteConfig'

const { data: services } = await useAsyncData('service-index', () => {
  return queryCollection('service').all()
})

const categoryLabel = serviceCategoryLabel

useHead({
  title: 'Service | tori-dev',
  meta: [
    { name: 'description', content: '顧問・講師・開発の3つのサービスを提供しています。' },
  ],
})
</script>
